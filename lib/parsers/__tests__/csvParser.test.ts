...
describe("parseCSV", () => {
  ...
  it("strips decorative quotes from the header so all columns are parsed", () => {
    ...
    expect(result[0].activeCalories).toBe(100);
  ...
  it("returns multiple rows in order", () => {
    ...
    expect(result[0].date).toEqual(new Date(Date.UTC(2026, 4, 2)));
    expect(result[1].date).toEqual(new Date(Date.UTC(2026, 4, 3)));
  });

  it("filters out rows with an invalid date", () => {
    const result = parseCSV(csv(makeRow("not-a-date", "100", "-", "-", "-", "-", "-", "-")));
...
describe("parseCSV range fields", () => {
  it("handles decimal min value — '46.22-130'", () => {
    ...
    expect(result[0].heartRate).toEqual({ min: 46.22, max: 130 });
  });
  ...
  it("treats single value as symmetric range — '58.51'", () => {
    ...
    expect(result[0].hrv).toEqual({ min: 58.51, max: 58.51 });
  });
...
describe("parseCSV sleep parsing", () => {
  it("parses hours and minutes — '3h 19m' → 199", () => {
    ...
    expect(result[0].sleep).toBe(199);
  });
  ...
  it("parses hours only — '2h' → 120", () => {
    ...
    expect(result[0].sleep).toBe(120);
  });
  ...
  it("returns null for dash sleep", () => {
    ...
    expect(result[0].sleep).toBeNull();
  });
});
...
describe("parseCSV number parsing", () => {
  it("strips commas — '1,109' → 1109 (value must be quoted in CSV)", () => {
    ...
    const result = parseCSV([HEADER, rowWithQuotedSteps].join("\n"));
    expect(result[0].steps).toBe(1109);
  });
  ...
  it("parses float cardio fitness", () => {
    ...
    expect(result[0].cardioFitness).toBe(41.38);
  });
});
